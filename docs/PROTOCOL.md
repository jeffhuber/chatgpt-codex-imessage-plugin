...
This document describes the JSON request/response protocol used between the
local iMessage MCP server and the macOS helper.
...
The MCP server runs as an unprivileged local process without Full Disk Access.
A LaunchAgent bridges it to the FDA-bearing helper:
