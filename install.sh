#!/bin/bash
# install.sh — one-time setup for the ChatGPT/Codex iMessage helper.
#
...
#      ~/Library/LaunchAgents/com.jeffhuber.chatgpt-codex-imessage.plist, then
#      bootstraps it.
#   7. Optionally installs the local OpenAI plugin and its MCP runtime.
...
WRAPPER_BIN="$BIN_DIR/chatgpt-codex-imessage-helper"
CONFIRM_SRC="$BIN_DIR/confirm_imessage_send.m"
CONFIRM_BIN="$BIN_DIR/chatgpt-codex-imessage-confirm"
PLIST_TEMPLATE="$INSTALL_ROOT/com.jeffhuber.chatgpt-codex-imessage.plist.template"
...
if [[ "$INSTALL_OPENAI_PLUGIN" != "0" && "$INSTALL_OPENAI_PLUGIN" != "1" ]]; then
    red "INSTALL_OPENAI_PLUGIN must be 0 or 1."
    exit 1
fi
...
if [[ "$INSTALL_OPENAI_PLUGIN" == "1" && \
      (! -f "$INSTALL_ROOT/.codex-plugin/plugin.json" || ! -x "$INSTALL_ROOT/install-plugin.sh") ]]; then
    red "Missing plugin manifest or executable install-plugin.sh"
    exit 1
fi
...
bold "ChatGPT/Codex iMessage helper installer"
...
for path in "$BRIDGE_ROOT" "$CONTROL_DIR" "$CONTROL_DIR/requests" \
...
chmod 700 "$INSTALL_ROOT" "$BRIDGE_ROOT" "$CONTROL_DIR" "$CONTROL_DIR/requests" \
...
if [[ "$INSTALL_OPENAI_PLUGIN" == "1" ]]; then
    ...
    yellow "  skipped OpenAI plugin installation (INSTALL_OPENAI_PLUGIN=0)"
fi
...
echo "       $WRAPPER_BIN"
echo
echo "  3. Select 'chatgpt-codex-imessage-helper' and make sure its toggle is ON."
echo "  4. (If prompted to quit and reopen anything, just click 'Later'.)"
echo
echo "Verify by asking ChatGPT/Codex: \"review my imessages over the last 2 days\""
echo
yellow "Note on sending (v0.3.0+):"
echo "  The first time you ask ChatGPT/Codex to send an iMessage, macOS will"
echo "  prompt 'chatgpt-codex-imessage-helper wants to control Messages.' Click OK."
