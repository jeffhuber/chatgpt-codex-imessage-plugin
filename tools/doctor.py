#!/usr/bin/env python3
"""Diagnose a ChatGPT/Codex iMessage helper installation without reading messages."""
...
def inspect_install(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.architecture:
        hardened = args.architecture == "hardened"
    else:
        hardened = code_root != bridge and code_root.exists() and code_root.stat().st_uid == 0
    ...
    executable_files = {
        "fda_wrapper": code_root / "bin" / "chatgpt-codex-imessage-helper",
        "confirmation_helper": code_root / "bin" / "chatgpt-codex-imessage-confirm",
    }
    ...
    if not args.skip_launchd:
        ...
        if launchctl is None:
            ...
            uid = os.getuid()
            result = run([launchctl, "print", f"gui/{uid}/com.jeffhuber.chatgpt-codex-imessage"])
    ...
    if not args.skip_plugin:
        plugin_root = pathlib.Path.home() / "plugins" / "chatgpt-codex-imessage-plugin"
        plugin_files = [
            plugin_root / ".codex-plugin" / "plugin.json",
            plugin_root / ".mcp.json",
            plugin_root / "bridge-path",
            plugin_root / "plugin_server" / "server.py",
            plugin_root / "scripts" / "bridge_paths.sh",
            plugin_root / "scripts" / "run-mcp-server.sh",
        ]
        files_ok = all(
            path.is_file() and not has_symlink_component(path) for path in plugin_files
        )
        checks["plugin_files"] = check(
            "pass" if files_ok else "fail",
            str(plugin_root),
        )

        mcp_python = bridge / "mcp-venv" / "bin" / "python"
        mcp_ok = mcp_python.is_file() and os.access(mcp_python, os.X_OK)
        checks["mcp_runtime"] = check(
            "pass" if mcp_ok else "fail",
            str(mcp_python),
        )

        marketplace_path = pathlib.Path.home() / ".agents" / "plugins" / "marketplace.json"
        marketplace_ok = False
        try:
            marketplace = json.loads(marketplace_path.read_text(encoding="utf-8"))
            marketplace_ok = any(
                item.get("name") == "chatgpt-codex-imessage-plugin"
                for item in marketplace.get("plugins", [])
                if isinstance(item, dict)
            )
        except (FileNotFoundError, json.JSONDecodeError, OSError):
            pass
        checks["personal_marketplace"] = check(
...
def main() -> int:
    ...
    parser.add_argument("--architecture", choices=("standard", "hardened"))
