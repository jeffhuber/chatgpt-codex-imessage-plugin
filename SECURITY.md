...
This document describes what the `chatgpt-codex-imessage-plugin` helper does with your Mac's
...
`chatgpt-codex-imessage-plugin` is an iMessage integration for ChatGPT/Codex (and compatible AI assistants) that:
...
`/Library/Application Support/ChatGPTCodexIMessage/users/<uid>/libexec` and compiles the wrapper
...
The ChatGPT/Codex installation has its own LaunchAgent, plist, executable names,
bridge, and hardened code root. The sibling Claude Cowork and Grok Bot helpers
can therefore run at the same time without sharing request queues, policy
files, responses, logs, nonces, or TCC identities. All three still rely on the
same macOS Messages database and Messages Automation surface.

### Automation → Messages (v0.3.0+)

Required to send. The first send triggers a one-time macOS prompt:
"chatgpt-codex-imessage-helper wants to control Messages." The grant lives
...
The helper communicates with the local MCP server via a **bridge folder** — a
mode-700 directory under the user's home directory (by default,
...
1. The AI asks the helper for a `send_preview` — the helper does
   ...
   triple.
2. ChatGPT/Codex shows the preview to you in chat; you approve.
...
The plugin skill also requires explicit approval after presenting the preview,
and the MCP send tool is annotated as a non-idempotent external write. Those
host-side controls are defense in depth; the native helper dialog remains the
...
The local STDIO MCP server also makes no outbound network requests. It has no
Full Disk Access or Automation grant. It submits narrowly typed requests to the
bridge, validates bridge ownership and mode, refuses symlink components, writes
mode-600 requests with atomic rename, bounds responses to 16 MiB, and deletes a
response immediately after parsing it. The MCP SDK is a pinned installation-time
dependency in a dedicated virtual environment; review `requirements-mcp.txt`
before installation.

When ChatGPT/Codex uses this skill, message content that ChatGPT/Codex reads
passes through OpenAI's normal pipeline, which means it reaches
OpenAI's servers as part of the conversation, subject to
OpenAI's standard data-handling terms. If you don't want a specific
...
- After install, verify the LaunchAgent plist under
  `~/Library/LaunchAgents/com.jeffhuber.chatgpt-codex-imessage.plist` points only
  at the selected wrapper: the source-clone binary in standard mode or the
...
4. System Settings → Privacy & Security → Automation →
   chatgpt-codex-imessage-helper → turn Messages off (or remove the entry
   entirely).

## Reporting a vulnerability

If you find a security issue, please do NOT open a public GitHub
issue. Email <jhuber+imessage-security@gmail.com> with details and, if possible, a
